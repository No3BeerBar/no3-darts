...
2. **Sign in** — same name + PIN. Session is an **httpOnly cookie** (`no3_player_session`) that stays on that tablet until **Sign out**, or until **2 minutes idle** when not mid-match (idle play/setup, and again after a match ends). Mid-match never idle-logs-out. See [`PLAY.md`](./PLAY.md).
...
4. **Resume** — in-progress games persist on the tablet, but registered seats must still be verified. After **Sign out**, a cleared session, or a **fresh page load / opening the board `/play` link / app restart**, Resume prompts for PIN again before scoring and idle play does not show sticky “Signed in” without PIN; guests-only / bot seats do not need PIN. Stay-signed-in between back-to-back matches on the same continuous kiosk session is unchanged. See [`PLAY.md`](./PLAY.md).
...
| `PASSPORT_DARTS_SHARED_SECRET` | **no3-darts** (app) when using No3Passport | Bearer secret for Passport integration routes (`link-challenge`, timed `challenges` upsert/standings/close). Required for those endpoints (401 if missing/wrong). Match the value configured in Passport. See [`CHALLENGES.md`](./CHALLENGES.md). |
...
| PUT/GET | `/api/integrations/passport/challenges*` | Timed challenge defs + standings for Passport. See [`CHALLENGES.md`](./CHALLENGES.md). |
