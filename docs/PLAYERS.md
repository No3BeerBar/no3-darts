...
4. **Resume** — in-progress games persist on the tablet, but registered seats must still be verified. After **Sign out**, a cleared session, or a **fresh page load / app restart**, Resume prompts for PIN again before scoring; guests-only / bot seats do not need PIN. Stay-signed-in between back-to-back matches on the same continuous kiosk session is unchanged. See [`PLAY.md`](./PLAY.md).
